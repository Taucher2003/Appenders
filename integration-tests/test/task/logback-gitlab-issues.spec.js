...
describe("GitLab Issue Appender for Logback", () => {
    // 3 times = drops all events without throwable
    it("Calls Rest API 3 times", async () => {
        const mock = jest.fn()
        await execute("GitLab-Issue", mock, logback);
    ...
    it("Adds configured label to body", async () => {
        const mock = jest.fn();
        await execute("GitLab-IssueLabels", mock, logback);
        expect(mock.mock.calls[0][0].body).toEqual({
            title: "java.lang.Throwable: Exception",
            description: expect.any(String),
            labels: ["logger"],
    ...
    it("Makes Issue confidential if configured", async () => {
        const mock = jest.fn();
        await execute("GitLab-IssueConfidential", mock, logback);
        expect(mock.mock.calls[0][0].body).toEqual({
            title: "java.lang.Throwable: Exception",
            description: expect.any(String),
            labels: [],
